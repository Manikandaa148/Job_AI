...
function HomeContent() {
    ...
    // Trigger search when sorts or filters change
    useEffect(() => {
        if (hasSearched) {
            performSearch(query, location, 1, true);
        }
    }, [selectedExperience, selectedPlatforms, selectedCompanySizes, sortBy]);
            ...
            <div className="max-w-7xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
                ...
                {jobs.length > 0 && (
                    <div className="flex flex-col sm:flex-row justify-between items-center mb-6">
                        <h2 className="text-xl font-semibold text-slate-900 dark:text-white mb-4 sm:mb-0">
                            Search Results ({jobs.length}+)
                        </h2>
                        <div className="flex items-center space-x-2">
                            <span className="text-sm text-slate-500 dark:text-blue-200/60">Sort by:</span>
                            <select
                                value={sortBy}
                                onChange={(e) => setSortBy(e.target.value)}
                                className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg text-sm px-3 py-2 text-slate-700 dark:text-blue-100 outline-none focus:border-blue-500 dark:focus:border-blue-400"
                            >
